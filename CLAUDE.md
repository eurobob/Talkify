...
- `Talkify/RemoteInput/` — the Siri Remote. **Read [`docs/siri-remote.md`](docs/siri-remote.md) before changing anything here**: how to add a spoken command, which shortcuts macOS will never accept from an app, and what to run when the remote goes quiet. The remote is three devices reached three ways. `SiriRemoteButtonMonitor` opens the buttons as a HID device, because macOS routes none of them through the CGEvent tap. `SiriRemoteTouchpad` reads the clickpad through the private `MultitouchSupport`, which is why the HID digitizer interface stays silent however hard the pad is swiped; `RemoteCursor` and `RingScroll` turn that into pointer and wheel. `Voice/` decodes the microphone from the Bluetooth link — macOS receives that audio and exposes it to nobody — with the root half in `helper/`, installed by the app itself through `SMAppService`. `Commands/` is the spoken vocabulary: a fixed grammar, deliberately, so a mis-hearing is reported rather than acted on.
