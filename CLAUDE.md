...
- `Talkify/RemoteInput/` — the Siri Remote. `SiriRemoteButtonMonitor` opens the remote as a HID device, because macOS routes none of its buttons through the CGEvent tap; its side button drives the same reducer events the keyboard trigger does, tagged with `DirectDictationController.TriggerSource.siriRemote`. `AudioInputDevice` picks the microphone that session records from. macOS never publishes the remote's own microphone — the remote sends Opus over the Bluetooth link and the system drops it — so a bridge process decodes it and publishes an ordinary input device, which is all this app ever sees. The trackpad is not here: it arrives through the private `MultitouchSupport.framework`, not HID.
